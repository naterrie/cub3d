...
void	minimap_full(t_data *data)
{
	...
	data->player.fov.x = cosf(t) * \
		data->player.direction.x + -sinf(t) * data->player.direction.y;
	data->player.fov.y = sinf(t) * \
		data->player.direction.x + cosf(t) * data->player.direction.x;
	display_game(data);
